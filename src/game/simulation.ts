...
export type Command = { lane: number };
...
export function applyCommand(s: GameState, command: Command) {
  ...
  // Revalidate against live geometry, not the snapshot sent before network latency.
  if (s.status === "running") emergencyDodge(s);
}
...
function emergencyDodge(s: GameState) {
  const attackX = WORLD.lanes[s.targetLane - 1];
  // Return to the attack as soon as the whole route is clear; no fixed hold timer.
  if (s.dodgeX !== null && !trajectoryThreats(s, attackX, 1.25).length)
    s.dodgeX = null;
  const targetX = s.dodgeX ?? attackX;
  if (!trajectoryThreats(s, targetX, 0.85).length) return;
  const currentRisk = trajectoryThreats(s, targetX, 1.25).length;
  const clearance = HITBOX.halfWidth + 18;
  const positions = [
    s.x,
    ...WORLD.lanes,
    ...s.bullets
      .filter((b) => b.enemy)
      .flatMap((b) => [b.x - clearance, b.x + clearance]),
  ];
  const candidates = [
    ...new Set(
      positions.map((x) =>
        Math.max(WORLD.lanes[0], Math.min(WORLD.lanes[4], x)),
      ),
    ),
  ].map((x) => ({
    x,
  ...
  candidates.sort(
    (a, b) =>
      ...
      Math.abs(a.x - attackX) - Math.abs(b.x - attackX),
  );
  const best = candidates[0];
  if (best.risk >= currentRisk) return;
  if (s.dodgeX === null) s.dodges++;
  s.dodgeX = best.x;
}

/** Attack deadlines include travel, weapon cooldown, projectile flight, and target HP. */
function attackWindow(s: GameState, laneX: number) {
  const target = s.enemies
    .filter((e) => e.x === laneX)
    .sort((a, b) => b.y - a.y)[0];
  if (!target) return null;
  const travel = Math.abs(s.x - laneX) / PLAYER_SPEED;
  const firstShot = travel + Math.max(0, s.shotTimer);
  const flight = Math.max(
    0,
    (WORLD.playerY - 28 - target.y - target.speed * firstShot) /
      (760 + target.speed),
  );
  const killTime = firstShot + flight + (target.hp - 1) * 0.25;
  const escapeTime = Math.max(
    0,
    (WORLD.playerY - 35 - target.y) / target.speed,
  );
  return {
    target_enemy_id: target.id,
    target_hp: target.hp,
    seconds_to_escape: +escapeTime.toFixed(2),
    estimated_seconds_to_destroy: +killTime.toFixed(2),
    escape_margin_seconds: +(escapeTime - killTime).toFixed(2),
    can_intercept: escapeTime > killTime,
  };
}
...
export function advance(s: GameState, dt: number) {
  ...
  emergencyDodge(s);
  const target = s.dodgeX ?? WORLD.lanes[s.targetLane - 1];
  s.x = positionAlongPath(s.x, target, dt);
  s.fire = s.enemies.length > 0;
  s.shotTimer -= dt;
...
export function radar(s: GameState) {
  const effectiveX = s.dodgeX ?? WORLD.lanes[s.targetLane - 1];
  const imminent = new Set(
    trajectoryThreats(s, effectiveX, 0.85).map((b) => b.id),
  );
  return {
    mission:
      "Destroy all enemy ships in 3 waves. Stay alive. Bullets travel vertically. You can move horizontally between 5 lanes and fire upward with unlimited ammunition. Weapons fire automatically whenever enemies remain. Choose the attack lane; prioritize interceptable enemies with the least escape margin. A local reflex makes short evasive moves and returns to your lane when the route is clear.",
    coordinates: {
      origin: "top left",
      x_direction: "right",
      y_direction: "down",
    ...
    weapons: "automatic continuous fire",
    emergency_reflex: {
      active: s.dodgeX !== null,
      effective_target_x: effectiveX,
      dodges: s.dodges,
    },
